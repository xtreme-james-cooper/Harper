...
sealed abstract class Type(name : => String) {
  ...
  def swap(mu : String, x : Type) : Type = this match {
    ...
    case Unknown(id, pendingSwaps) if (!pendingSwaps.contains(mu)) => Unknown(id, pendingSwaps + (mu -> x))
    case Unknown(id, pendingSwaps) => Unknown(id, pendingSwaps) //ignore it; it won't cause a change
  }
  ...
  def swap(i : Int, x : Type) : Type = this match {
    ...
    case Unknown(id, pendingSwaps) if i == id => x.swap(pendingSwaps)
    case Unknown(id, pendingSwaps)            => Unknown(id, pendingSwaps)
  }
  ...
  def ~=~(t2 : Type) : List[(Type, Type)] = (this, t2) match {
    case (Unknown(i, pendingSwaps), b)          => (Unknown(i, pendingSwaps), b.swap(pendingSwaps)) :: Nil
    case (a, Unknown(i, pendingSwaps))          => (Unknown(i, pendingSwaps), a.swap(pendingSwaps)) :: Nil
...
case class Unknown(id : Int, pendingSwaps : Map[String, Type]) extends Type("unknown " + id + " being used")
