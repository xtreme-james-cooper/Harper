...
object Main {
  ...
  def main(args : Array[String]) : Unit = {
    ...
    test("null : Unit = (); atomic return null")

    test("plus(a : Nat, b : Nat) : Nat = case a of {Z -> b | S(n) -> S(((plus n) b))}; atomic return ((plus 2) 3)")

    test("swap(p : (Unit, Nat)) : (Nat, Unit) = case p of {(a, b) -> (b, a)};"
      + "null : Unit = ();"
      + "atomic return (swap (null, S(Z)))")
    ...
    test("swap(p : ((Unit, Nat), Nat)) : (Unit, (Nat, Nat)) = case p of {((a, b), c) -> (a, (b, c))};"
      + "null : Unit = ();"
      + "atomic return (swap ((null, S(Z)), 2))")

    test("ifC(b : Bool, e1 : Nat, e2 : Nat) : Nat = case b of{inl x -> e1 | inr x -> e2};"
      + "plus(a : Nat, b : Nat) : Nat = case a of {Z -> b | S(n) -> S(((plus n) b))};"
      + "atomic return ((plus (((ifC true) S(Z)) Z)) (((ifC false) S(S(S(S(Z))))) S(S(Z))))")

    test("atomic return case inr inl () of {"
      + "   inl (S(_), y) -> y |"
      + "   inl (Z, y) -> S(y) |"
      + "   inr inl () -> 2 |"
      + "   inr inr x -> S(S(S(x))) }")
    ...
    test("ifC(b : Bool, e1 : (Nat -> Nat), e2 : (Nat -> Nat)) : Nat = case b of{inl x -> (e1 0) | inr x -> (e2 0)};"
        + "atomic return (((ifC true) \\x:Nat . 3) \\x:Nat . throw bleh)")
  
    test("ifC(b : Bool, e1 : (Nat -> Nat), e2 : (Nat -> Nat)) : Nat = case b of{inl x -> (e1 0) | inr x -> (e2 0)};"
        + "atomic return (((ifC false) \\x:Nat . 3) \\x:Nat . throw bleh)")
    
    test("ifC(b : Bool, e1 : (Nat -> Nat), e2 : (Nat -> Nat)) : Nat = case b of{inl x -> (e1 0) | inr x -> (e2 0)};"
        + "atomic return try (((ifC true) \\x:Nat . 3) \\x:Nat . throw bleh) catch 2")
  
    test("ifC(b : Bool, e1 : (Nat -> Nat), e2 : (Nat -> Nat)) : Nat = case b of{inl x -> (e1 0) | inr x -> (e2 0)};"
        + "atomic return try (((ifC false) \\x:Nat . 3) \\x:Nat . throw bleh) catch 2")

    test("plus(a : Nat, b : Nat) : Nat = case a of {Z -> b | S(n) -> S(((plus n) b))};"
        + "atomic decl a := 0 in decl b := 3 in bind x <- command !b; a := ((plus x) 2); bind y <- command !a; return y")

    test("plus(a : Nat, b : Nat) : Nat = case a of {Z -> b | S(n) -> S(((plus n) b))};"
      + "sum(x : Nat, y : Nat) : Nat = decl a := ((plus x) y) in bind b <- command !a; return b;"
      + "atomic decl a := 2 in decl b := 3 in bind aa <- command !a; bind bb <- command !b; "
      + "   bind x <- ((sum aa) bb); a := ((plus x) 1); bind y <- command !a; return y")
  }
