...
object PatternCompiler {
  ...
  def run(v1 : Value, rs : List[(Pattern, String)]) : (String, Map[String, Value]) = {
    n = 0
    PatternCPU.run(v1, compileFullPattern(rs))
  }

  //TODO eliminate succtag when the two CPUs are folded together
  
  def compileFullPattern(rss : List[(Pattern, String)]) : List[PatternOpcode] = compileRules(rss) ++ List(Label("donePattern"), Exit)

  def compileRules(rss : List[(Pattern, String)]) : List[PatternOpcode] = rss match {
  ...
  def compileRule(p : Pattern, b : String, succtag : String, failtag : String) : List[PatternOpcode] = {
  ...
  //TODO elimiate from here down
  
  def fakerun(v1 : Value, rs : List[Rule]) : (Expr, Map[String, Value]) = {
    n = 0
    PatternCPU.fakerun(v1, fakecompileFullPattern(rs))
  }

  def fakecompileFullPattern(rss : List[Rule]) : List[PatternOpcode] = fakecompileRules(rss) ++ List(Label("donePattern"), Exit)

  def fakecompileRules(rss : List[Rule]) : List[PatternOpcode] = rss match {
    case Nil => List(Thrw("No pattern match found"))
    case Rule(p, b) :: rs => {
      val failtag = "failure" + n
      n = n + 1
      List(SetReg(R_BIND_SP, 0), SetReg(R_VAL_SP, 0), SetReg(R_DUMMY, 0)) ++ pushVal(R_DUMMY) ++
        compileRule(p, b, "donePattern", failtag) ++ List(Label(failtag)) ++ fakecompileRules(rs)
    }
  }
  ...
  def compileRule(p : Pattern, b : Expr, succtag : String, failtag : String) : List[PatternOpcode] = {
    ...
    List(ValPop(R_HEAP_B), ValPop(R_HEAP_A), ValPop(R_TAG)) ++ (p match {
      case WildPat    => List(FakeSetRetval(b), Jump(succtag))
      case TrivPat    => putVIntoRegs ++ List(FakeSetRetval(b), Jump(succtag))
      case VarPat(x)  => List(PushVRetStack(x), FakeSetRetval(b), Jump(succtag))
      case ZPat       => putVIntoRegs ++ List(JIfNEq(R_TAG, ZTAG, failtag), FakeSetRetval(b), Jump(succtag))
      case SPat(sp)   => putVIntoRegs ++ List(JIfNEq(R_TAG, STAG, failtag)) ++ compileRule(sp, b, succtag, failtag)
      case InLPat(sp) => putVIntoRegs ++ List(JIfNEq(R_TAG, INLTAG, failtag)) ++ compileRule(sp, b, succtag, failtag)
      case InRPat(sp) => putVIntoRegs ++ List(JIfNEq(R_TAG, INRTAG, failtag)) ++ compileRule(sp, b, succtag, failtag)
      case PairPat(p1, p2) => {
        ...
        n = n + 1
        val subOps2 = compileRule(p1, b, subsucc2, failtag)
        putVIntoRegs ++ subOps1 ++ List(Label(subsucc1)) ++ subOps2 ++ List(Label(subsucc2), FakeSetRetval(b), Jump(succtag))
      }
    })
  }
