...
object CommandCompiler extends Compiler[CmdStack, Command, Value] {
  ...
  override def evalStep(c : Command) : Unit = c match {
    case Ret(e) => target = Return(ExprCompiler.run(e, env))
    ...
    case SetCmd(x, e, m) => {
      updateMemory(x, ExprCompiler.run(e, env))
      target = Eval(m)
    }
    case Decl(x, e, m) => {
      pushMem(Map(x -> ExprCompiler.run(e, env)))
      target = Eval(m)
    }
  }
