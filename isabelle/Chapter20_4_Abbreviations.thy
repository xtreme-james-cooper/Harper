...
lemma "type_free_vars Nat = {}" by simp
...
lemma "typecheck env Zero Nat" by (simp add: type_subst_def)
...
lemma "typecheck env n Nat ==> typecheck env (Succ n) Nat" by (simp add: type_subst_def)
...
proof -
  ...
  moreover hence "typecheck env (Unfold et) (type_subst (Sum Unit (Tyvar 0)) Nat)" by simp
  ultimately show "typecheck env (Case (Unfold et) (incr_from 0 e0) e1) t" by (simp add: type_subst_def)
qed
...
proof -
  have "eval (Case (InL Unit Nat Triv) (incr_from 0 e0) e1) (subst (incr_from 0 e0) Triv)" by (metis ecs2 is_val.simps(5))
...
qed
lemma "is_val e ==> EX e'. eval (IsZ (Succ e) e0 e1) e' & eval e' (subst e1 e)" 
proof -
  assume X: "is_val e"
  hence "eval (Case (Unfold (Succ e)) (incr_from 0 e0) e1) (Case (InR Unit Nat e) (incr_from 0 e0) e1)" by simp
  moreover with X have "eval (Case (InR Unit Nat e) (incr_from 0 e0) e1) (subst e1 e)" by simp
  ultimately show "EX e'. eval (IsZ (Succ e) e0 e1) e' & eval e' (subst e1 e)" by auto
qed
...
by (induction env e t rule: typecheck.induct, auto simp add: canonical_unit type_subst_def) 
